...
function onLoad() {
    ...
    // simple dirty debouncer
    var deb = setInterval(dTimer, 500);
    function dTimer() {
        if (floodnew > floodold + 100) {
           ...
           debExit();
            ...
            if (window.location.hash) {
                ...
                var index = nodesonline.indexOf(hash);
                if (index > -1) {
                    showNodeLinks(hash);
                } else {
                    document.getElementById("nodestath").innerHTML = "Node ID: "+hash+" not found";
                }
    ...
    var socket = new WebSocket(((window.location.protocol === "https:") ? "wss://" : "ws://") + "116.202.49.178" + "/datastream");
    ...
    function setNodeOffline(idA) {

        var index = nodesoffline.indexOf(idA);
        if (index == -1) {
            //console.log("setNodeOffline: " + idA);
            nodesoffline.push(idA);
            if (rendered == 1) {
                calculateDiscNodes();
                document.getElementById("streamstat").innerHTML = "addedToOfflinepool: " + idA;
                document.getElementById("stat").innerHTML = "nodes online: " + nodesonline.length + "(" + nodesdisconnected.length + ")" + " - IDs: " + nodepool.length + " - edges: " + connections.length;
            }
        }

        var index2 = nodesonline.indexOf(idA);
        if (index2 > -1) {
            //console.log("removedFromOnlinepool " + idA);
            nodesonline.splice(index2, 1);
            graph.removeNode(idA);

            if (rendered == 1) {
                calculateDiscNodes();
                document.getElementById("streamstat").innerHTML = "removedFromOnlinepool: " + idA;
                document.getElementById("stat").innerHTML = "nodes online: " + nodesonline.length + "(" + nodesdisconnected.length + ")" + " - IDs: " + nodepool.length + " - edges: " + connections.length;
            }
        }
    ...
    function showNodeLinks(selected) {
        ...
        graph.forEachLinkedNode(selected, function (linkedNode, link) {
            ...
            if (links.startsWith(nodeid) === true) {
                document.getElementById("out").innerHTML += /*link.data.substr(0, 40)*/ "NODE" + " &rarr; " + link.data.substr(64, 127) + "<br>";
                outc++;
                ...
                document.getElementById("in").innerHTML += link.data.substr(0, 63) + " &rarr; " + /*link.data.substr(40, 40)*/ "NODE" + "<br>";
                inc++;
...
//     var conne = graph.getLink(string.substr(1, 40), string.substr(41, 40));
